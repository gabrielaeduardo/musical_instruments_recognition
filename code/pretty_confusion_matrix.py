...
def configcell_text_and_colors(
    ...
    if (col == (ccl - 1)) or (lin == (ccl - 1)):
        ...
        if col == lin:
            ...
            facecolors[posi] = [1.0, 1.0, 1.0, 1.0]
        else:
            oText.set_color("r")
    return text_add, text_del, facecolors[posi]
...
def pp_matrix(
    ...
    if pred_val_axis in ("col", "x"):
        xlbl = "Classe predita"
        ylbl = "Classe verdadeira"
    else:
        xlbl = "Classe verdadeira"
        ylbl = "Classe predita"
        df_cm = df_cm.T
    ...
    bla = ax.collections[0].axes
    # titles and legends
    # ax.set_title("Confusion matrix")
    ax.set_xlabel(xlbl)
    ax.set_ylabel(ylbl)
    plt.tight_layout()  # set layout slim
    plt.savefig(path, bbox_inches='tight',  dpi=256)
    plt.show()